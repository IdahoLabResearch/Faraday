...
from . import matrices as helpers
...
def compute_GCV(log_lambda, A_re, A_im, Z_re, Z_im, M, data_used, induct_used):
    ...
    if helpers.is_pd(A_in)==False: 
        A_in = utilities.nearest_PD(A_in) 
...
def compute_re_im_cv(log_lambda, A_re, A_im, Z_re, Z_im, M, data_used, induct_used):
    ...
    H_re, c_re = utilities.quad_format_separate(A_re, Z_re, M, lambda_value)
    H_im, c_im = utilities.quad_format_separate(A_im, Z_im, M, lambda_value)
...
def compute_kf_cv(log_lambda, A_re, A_im, Z_re, Z_im, M, data_used, induct_used):
    ...
    for train_index, test_index in kf.split(Z_re):
        ...
        H_combined, c_combined = utilities.quad_format_combined(A_re_train, A_im_train, Z_re_train, Z_im_train, M, lambda_value)
...
def compute_LC(log_lambda, A_re, A_im, Z_re, Z_im, M, data_used, induct_used):
    ...
    if helpers.is_pd(A_in_d)==False:
        A_in_d = utilities.nearest_PD(A_in_d)
